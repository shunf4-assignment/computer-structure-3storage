...
/* ------------ SD AND SPI CONTROLLER INSTANTIATION SECTION ------------ */
    ...
    sd_controller sdcon(
        ...
        .oStartOk(start_ok),
        .oSpiEn(en_spi),
        .oSpiClk74En(en_clk74),
        .iSpiClk74Ok(done_clk74),
        .oSpiTxEn(en_tx),
        .iSpiTxOk(done_tx),
        .oSpiTxData(data_tx),
        .oSpiRxEn(en_rx),
        ...
        .iSpiRxData(data_rx),
        .oSpiClk8En(en_clk8),
        .iSpiClk8Ok(done_clk8),
        .iReadEn(en_read),
        .iReadAddr(addr_read),
        .iReadSectorNum(sectorcnt_read),
        .oReadOk(done_read),
        .oReadData(data_read),
        .oReadDataValid(data_read_valid),
        .oReadDataASectorDone(data_read_sectordone),
        .cmd0ResetOk(cmd0_r_ok),
        .cmd1LegacyInitOk(cmd1_r_ok),
        .cmd8AskVoltageOk(cmd8_r_ok),
        .cmd55AppCommandOk(cmd55_r_ok),
        .acmd41InitOk(acmd41_r_ok),
        .error(sd_err),
        .sdState(sd_state),
        .oDebugInfo(sd_debug_info),
        .oDebugInfoAvailable(sd_debug_info_en),
        .acmd41InitAbandoned(acmd41_bypassed),
        .cmd58ReadOcrNotBusy(cmd58_prepared),
        .cmd58ReadOcrCCS(cmd58_ccs),
        .responseStart(resp_startvalid),
        .responseByteCounter(resp_bytecnt)
    );
...
/* ------------ INFO DISPLAY SECTION ------------ */
    ...
    assign sd_idle = sd_state == S_SD_IDLE;
    ...
    sd_controller_disp sdctrldisp(
        ...
        .resp_startvalid(resp_startvalid),
...
//BTNL, BTNR 捕获器
    ...
    always @ (posedge CLK100MHZ) begin
        ...
        if(rst) begin
            ...
            is_fdd_not_hddmbr <= 0;
            curr_r <= 0;
        end else if(sd_err && main_state != 0 && main_state != S_MAINERR && soft_rst == 0)
            main_state <= S_MAINERR;
        else begin
